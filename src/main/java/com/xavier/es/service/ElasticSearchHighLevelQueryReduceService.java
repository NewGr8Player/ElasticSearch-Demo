...
@Slf4j
@Service
public class ElasticSearchHighLevelQueryReduceService {
	...
	public void commonReduce(String tableName, CanalEntry.RowData rowData, CanalEntry.EventType eventType) throws Exception {
		...
		if (!ElasticsearchUtil.isIndexExist(PT_PETITION_HIGH_LEVEL_REDUCE)) {
			...
			log.info("创建索引:{}-{}", PT_PETITION_HIGH_LEVEL_REDUCE, "ElasticSearchHighLevelQueryReduceService#commonReduce");
		}
		switch (eventType) {
			case INSERT:
	...
	public void caseReduce(String tableName, CanalEntry.RowData rowData, CanalEntry.EventType eventType) throws Exception {
		...
		if (!ElasticsearchUtil.isIndexExist(PT_PETITION_HIGH_LEVEL_REDUCE)) {
			ElasticsearchUtil.createIndex(PT_PETITION_HIGH_LEVEL_REDUCE);
			log.info("创建索引:{}-{}", PT_PETITION_HIGH_LEVEL_REDUCE, "ElasticSearchHighLevelQueryReduceService#caseReduce");
		}
		switch (eventType) {
			case INSERT:
				ElasticsearchUtil.addData(JSONObject.parseObject(JSON.toJSONString(dataMap)), PT_PETITION_HIGH_LEVEL_REDUCE, PT_PETITION_HIGH_LEVEL_REDUCE, (String) dataMap.get("id"));
